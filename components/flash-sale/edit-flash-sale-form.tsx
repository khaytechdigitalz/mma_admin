...
import { Percent, Banknote, Search, X, Tag, Loader2, Trash2 } from "lucide-react";
...
export default function EditFlashSaleForm() {
  ...
  const fetchFlashSaleDetails = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        ...
        setStatus(isActive ? statusOptions[0] : statusOptions[1]);
      ...
      toast.error(err?.response?.data?.message || "Failed to load flash sale details.");
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  const toggleSelectProduct = (product: ProductItem) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const response = await apiClient.put(`/flash-sales/${id}`, payload);

      toast.success(response.data?.message || "Flash sale updated successfully!");
      router.push("/flash-sales");
    } catch (err: any) {
      console.error("Error updating flash sale:", err);
      toast.error(err?.response?.data?.message || "Failed to update flash sale.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="w-full bg-white rounded-2xl mx-auto p-12 flex items-center justify-center">
        <div className="flex flex-col items-center gap-2">
          <Loader2 className="w-8 h-8 animate-spin text-teal-600" />
          <span className="text-xs font-medium text-gray-500">Loading flash sale details...</span>
        </div>
      </div>
    );
  }
