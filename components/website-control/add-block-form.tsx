...
import { Loader2, Image as ImageIcon, Type as TypeIcon, Upload, Link as LinkIcon, Plus, Trash2, HelpCircle, Layers } from "lucide-react";
...
interface MultiItem {
  ...
  imagePreview?: string;
  head: string;
  subtext: string; // WYSIWYG content
}
...
export default function AddBlockForm() {
  ...
  // Multi Array handlers
  const handleAddMultiField = () => {
    setMultiList([...multiList, { image: "", imageFile: null, imagePreview: "", head: "", subtext: "" }]);
  };

  const handleRemoveMultiField = (index: number) => {
    const updated = multiList.filter((_, i) => i !== index);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (type === "question") {
      ...
      if (!isValidFaq) {
        return toast.error("Please add at least one complete Question and Answer pair.");
      }
      ...
      if (!isValidMulti) {
        return toast.error("Please add at least one complete item containing content.");
      }
      // Check if any multi item contains a file upload
    ...
    if (isMultipart) {
      if (type === "image" && imageFile) {
        ...
        // Append JSON structure and any individual image files if needed
        formData.append("value", JSON.stringify(multiList.map(({ imageFile, imagePreview, ...rest }) => rest)));
        multiList.forEach((item, idx) => {
          if (item.imageFile) {
            formData.append(`multi_images[${idx}]`, item.imageFile);
          }
        });
      } else {
    ...
    try {
      ...
      toast.success(response.data?.message || "Frontend block created successfully!");
      router.push("/website-control");
    } catch (err: any) {
      console.error("Error creating block:", err);
      toast.error(err?.response?.data?.message || "Failed to create frontend block.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="w-full bg-white rounded-2xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-500/20 space-y-6">
          ...
          {/* Dynamic Content Value Field based on Type */}
          <div className="pt-2">
            {type === "image" ? (
                ...
                <div className="space-y-4">
                  {multiList.map((item, index) => (
                    <div key={index} className="p-4 rounded-xl border border-gray-200 bg-gray-50/50 relative space-y-4">
                      ...
                      {/* Image Upload/URL for Multi item */}
