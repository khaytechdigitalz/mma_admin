...
export default function EditBlogForm() {
  ...
  const loadBlog = useCallback(async () => {
    if (!blogId) {
      toast.error("No blog selected to edit.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      const blog: BlogDetails = res.data?.data || res.data;

      setTitle(blog.title || "");
      setBody(blog.body || "");
      setStatus(blog.status ? statusOptions[0] : statusOptions[1]);
      if (blog.image) {
        setImagePreview(getFullImageUrl(blog.image) || "");
      }
    } catch (err: any) {
      console.error("Failed to load blog:", err);
      toast.error(err?.response?.data?.message || "Failed to load blog details.");
    } finally {
      setIsLoading(false);
    }
  }, [blogId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasNewFile = Boolean(imageFile && imageInputMode === "upload");

    const formData = new FormData();
    formData.append("title", title.trim());
    formData.append("body", body.trim());
    formData.append("status", statusValue ? "1" : "0");
    // Laravel-style APIs commonly require this for multipart PUT-as-POST updates.
    formData.append("_method", "PUT");

    if (hasNewFile && imageFile) {
    ...
    try {
      ...
      const response = hasNewFile
        ? await apiClient.post(`blogs/${blogId}`, formData, {
        ...
        : await apiClient.put(`blogs/${blogId}`, {
            ...
            body: body.trim(),
            status: statusValue,
            image: imageInputMode === "url" && imageUrl.trim() ? imageUrl.trim() : undefined,
          });
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full bg-white rounded-2xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="flex gap-4 flex-col sm:flex-row sm:items-center justify-between">
        <PageHeader title="Edit Blog" backHref="/blog-control" className="gap-4" />
        <div className="w-40">
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white rounded-2xl p-4 sm:p-6 border border-gray-500/25 space-y-6">
          <h2 className="text-lg font-bold text-gray-900 mb-4 sm:mb-6">Blog Information</h2>
